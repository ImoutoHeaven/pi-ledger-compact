...
Item lists and searches return `items`, `totalMatches`, `returnedCount`, `snapshotThrough`, and `nextCursor`. `order` is `newest` by default and also accepts `oldest`. Cursors bind the branch snapshot, tool, filters, projection, ordering, and match options. Later activity leaves that snapshot stable. Unknown fields and invalid ranges produce explicit errors.
...
`history_list_windows` accepts the shared filter, ordering, limit, and cursor. It includes initial and committed windows even when empty. `entryCount` and first/last entry IDs describe native window attribution; `matchedEntryCount`, `kindCounts`, `failedToolResults`, and `imageCount` use the same filter as item listing. Tool-call counts count invocations; other kinds count entries. Latest user previews quote matching inputs. Checkpoint counts and excerpts describe snapshots grouped by original source window independently of the filter. Successful history results obey `LEDGER_CONTEXT_READ_TOKENS`.
...
`history_read` defaults to `view: "entry"` and accepts `projection`, an optional original `contentIndex`, and UTF-16 `offset`/`length` pagination with `nextOffset`. `view: "exchange"` follows a tool call and every matching result up to reuse of that call ID; a result anchor selects its nearest preceding invocation. `contentIndex` can select one invocation in a multi-call message. Missing calls/results are reported explicitly. `view: "neighbors"` reads chronological log entries around an anchor, with `before` and `after` defaulting to 2 and bounded at 20. Related views use `limit`/`cursor`, default to oldest-first order, and preserve a snapshot. Neighborhoods express log proximity. The session log remains the evidence source.
