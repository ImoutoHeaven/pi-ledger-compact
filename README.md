...
Call `checkpoint` with the complete current working state in `ledger` and optional `sourceQuotes`: exact, case-sensitive phrases copied from source messages, ordered by recovery priority. Essential facts and constraints belong in the ledger. A successful save replaces the baseline and its entire source list; omitting `sourceQuotes` saves an empty list. Earlier versions remain in history. The receipt reports the saved entry, request position, size, persistence scope, and each quote's `matched`, `ambiguous`, or `unmatched` result. Pi controls compaction timing.
...
After compaction, each main-agent request projects the latest checkpoint, its matching delta and bounded source excerpts into the recovery summary. A new checkpoint immediately becomes the baseline; earlier versions remain readable in history. The persisted compaction packet keeps recent-tail source IDs and optional custom instructions. Each request renders the current ledgers' selected sources with branch edits and a shared source budget, while persisted records remain unchanged. Excerpts carry expansion references for `history_read`; omitted or unavailable items are counted. Recovery ownership is checked against the current branch before projection.
...
Reminders are triggered by accumulated work volume or budget pressure. After a tool batch, they use pi's native steering; when a run has already ended, pending reminders wait for the next normal user request. Before each model request, reminder applicability is checked again: volume notices expire when their window or checkpoint origin changes, and budget notices reflect current headroom and urgency. The request keeps at most one applicable notice per reason kind. Reminders retain their custom identity in the session log and Pi maps them to user messages for model input, including when another extension forces the system prompt. The text identifies an automated maintenance request, asks for a complete `checkpoint`, and tells the agent to resume after a successful save. Measurements and entry IDs remain in metadata. Source selection excludes maintenance reminder records; request snapshot IDs describe chronological boundaries and may point to a reminder.
