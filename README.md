...
Quotes resolve at save time against original user, assistant and ordinary tool messages on the current branch through the generating request snapshot, with context edits applied. Generated recovery records and maintenance tool traffic are excluded. Each quote records its total matching entry count and the first four candidates in chronological order, including their IDs, edit provenance and first match offsets. Ambiguous and unmatched quotes accompany a successful ledger save. The agent decides which sources matter; the extension performs matching and bounded rendering.
...
Pi's `buildSessionProjection()` supplies the effective messages for request projection, retained-tail selection and automatic delta history. Current-branch context edits control omission and replacement. LLM-selected sources produce bounded excerpts around quote matches; overlapping excerpts merge and each keeps its role, source ID and truncation status. Edited sources use their current text and report changes since selection; omitted sources stay out of recovery. New replacements of retained earlier evidence qualify for the next delta. Replacement text cites its edit record, which also owns replacement-image references. Original entries, checkpoints and delta records remain the history and provenance source.

The delta generator sees source entry IDs. It can end its text with a new line starting `<source-references>[{"entryId":"supplied-id","quote":"optional source phrase"}]</source-references>`. The parsed block becomes separate `sourceReferences` metadata. Quotes use the same case-sensitive whitespace matching as checkpoints and recovery excerpts, with offsets into the original source text. Each generated delta replaces its cumulative source list; omission saves an empty list. IDs resolve only among supplied sources, including current replacement-edit IDs. Invalid block syntax participates in the bounded generation retry policy; unresolved selectors are recorded alongside valid delta text.
...
| `projections` | `reference`, `checkpoint-ledger`, `delta-ledger`, `filtered-entry`, `source-excerpt`, or `context-edit`, with source IDs and supplied/total UTF-16 lengths. Source excerpts may start within an entry. `context-edit` also records `editEntryId` for the applied replacement. |
| `omittedRanges` | Inclusive ranges within `historyScope` left out of the bounded input, with entry counts. |
| `excludedRanges` | Inclusive ranges excluded as `maintenance`, `structural-metadata`, `context-omitted` (explicit omission), or `inactive-context` (outside Pi's active projection). Selected historical sources and substantive history-tool results remain eligible delta evidence. |

The delta's `scope` spans its cumulative target interval. Its input record's `historyScope` spans the new history considered for that request. Selected historical sources may precede this interval. Earlier delta text counts as a summary projection; its original sources retain their earlier input records. Relevance, understanding and verification remain judgments supported by evidence during ordinary agent work.
...
Each recovery summary uses the checkpoint and delta selected by its compaction. Subsequent checkpoint calls and receipts remain at their original positions in the working history; the latest successful checkpoint becomes the base for the next compaction. With stable settings and source content, ordinary work and checkpoint saves preserve the earlier recovery summary. Its selected source excerpts reflect explicit branch edits and the configured source budget. The persisted packet keeps recent-tail source IDs and optional custom instructions. Excerpts carry `history_read` references and omission counts. Recovery ownership is checked against the current branch before projection, and `recoveryBasis` identifies the checkpoint and delta actually supplied by that summary.
...
Successful image reads enter the session as tool-result image blocks. Main-agent requests preserve the images and tool pairing supplied by Pi's projection. Pi handles provider-specific image representation and main-request capacity. The extension's image limits apply when producing a `history_read` result.
...
| `LEDGER_CONTEXT_SOURCE_TOKENS` | `max(1, floor(window × 0.05))` | Shared estimated token limit for selected source excerpts, locator status and compaction focus |
| `LEDGER_CONTEXT_TAIL_TOKENS` | `max(1, floor(window × 0.05))` | Estimated token limit for compaction tail selection and recovery references |
...
Pi owns main-agent request capacity and provider encoding. Ordinary messages, tool-call arguments, tool results and images retain Pi's projected content. Extension limits apply to checkpoints, generated deltas, recovery material and history-tool outputs. Image reads obey the total `LEDGER_CONTEXT_READ_TOKENS` limit.

Checkpoint and delta text each have a `65,536` UTF-8 byte limit. Each accepts at most `8` source selectors; quotes are at most `512` UTF-16 code units, and each selector retains at most `4` matching entries. Source excerpt windows start at up to `1024` code units per match before overlap merging and total-budget clipping. History search text allows up to `8,192` UTF-8 bytes, identifiers up to `1,024` UTF-16 code units, and history pages up to `100` results. Text reads accept a maximum length of `65,536` UTF-16 code units. These input limits apply alongside the output budgets.
...
- Recovery material includes complete checkpoint and delta text within their configured ledger limits. Incompatible ledger limits stop recovery rendering or compaction while preserving the saved records.
