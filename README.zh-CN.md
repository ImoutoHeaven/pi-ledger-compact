...
条目列表和搜索结果包含 `items`、`totalMatches`、`returnedCount`、`snapshotThrough` 和 `nextCursor`。`order` 默认 `newest`，也支持 `oldest`。游标绑定分支快照、工具、过滤条件、投影、顺序和匹配选项，后续活动保持该快照稳定。未知字段和无效范围会返回明确错误。
...
`history_list_windows` 接受共用过滤条件、顺序、条数和游标，包含初始窗口及条目数为零的已提交窗口。`entryCount` 和首尾条目 ID 描述原生窗口归属；`matchedEntryCount`、`kindCounts`、`failedToolResults`、`imageCount` 使用与条目列表相同的过滤条件。工具调用按调用数统计，其他种类按条目数统计。最新用户预览引用匹配的输入原文；checkpoint 数量和摘录独立于过滤条件，按原始来源窗口组织。成功返回的历史结果遵守 `LEDGER_CONTEXT_READ_TOKENS` 预算。
...
`history_read` 默认使用 `view: "entry"`，接受 `projection`、可选的原始 `contentIndex`，以及按 UTF-16 计量的 `offset`/`length` 正文分页，返回 `nextOffset`。`view: "exchange"` 读取工具调用与该调用 ID 再次使用之前的每个匹配结果；从结果出发时定位最近的前序调用。`contentIndex` 可在多调用消息中指定一个调用。缺失的调用或结果会明确标记。`view: "neighbors"` 读取锚点附近的日志条目，`before`、`after` 默认 2，上限 20。关联视图使用 `limit`/`cursor` 分页，默认从旧到新，并保持快照；邻近视图表达日志位置关系。完整会话日志作为证据来源。
