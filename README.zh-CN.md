...
保存检查点后，当前运行继续执行。pi 控制压缩时机。每次自动压缩和手动 `/compact [instructions]` 都使用当前模型、宿主认证、上一个可用账本、最新任务及后续有界历史，尝试一次账本更新，并持续等待响应或错误；期间可由用户取消。有效结果保存为检查点并用于恢复。生成失败或输出无效时，恢复上一个可用检查点，同时明确提醒账本可能过期，并提供恢复引用。该检查点不可用或超过当前账本预算时，恢复引导明确要求从会话历史恢复状态。用户取消会取消压缩；检查点写入失败时停止恢复，需重新打开持久会话。
